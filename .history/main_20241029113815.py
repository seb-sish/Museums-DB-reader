...
class MyApp(QtWidgets.QMainWindow):
    ...
    def addItem(self):
        ...
        self.statusBar().showMessage(f"Добавлена {row+1} строка", 300000)
    ...
    def removeRow(self):
        ...
        self.statusBar().showMessage(f"Удалена {index+1} строка", 300000)
    ...
    def open_file(self):
        try:
            ...
            self.load_database()
            self.statusBar().showMessage(f"Открыт файл {path}", 300000)
        except FileNotFoundError:
            print("file not open!")
            self.statusBar().showMessage(f"Файл НЕ был открыт!", 300000)
    ...
    def concatinate_file(self):
        try:
            ...
            self.statusBar().showMessage(f"Исходная таблица объеденена с файлом {path}", 300000)
        except FileNotFoundError:
            print("file not open!")
            self.statusBar().showMessage(f"Файл НЕ был открыт!", 300000)
    ...
    def create_new_file(self):
        try:
            ...
            self.load_database()
            self.statusBar().showMessage(f"Создан файл {path}", 300000)
        except FileNotFoundError:
            print("file not open!")
            self.statusBar().showMessage(f"Файл НЕ создан!", 300000)
    ...
    def save_file(self):
        ...
        self.statusBar().showMessage("Файл сохранен", 300000)
    ...
    def saveas_file(self):
        ...
        if path != '' and path.split(".")[-1] in ("json"):
            ...
            self.statusBar().showMessage(f"Файл сохранен как {path}", 300000)
        else:
            print("file not save!")
            self.statusBar().show showMessage(f"Файл НЕ сохранен!", 300000)

    @QtCore.Slot()
    def clear_table(self):
        while self.table.rowCount() > 0:
            self.table.removeRow(0)
        self.statusBar().showMessage(f"Таблица очищена", 300000)

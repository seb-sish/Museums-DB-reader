...
class MyApp(QtWidgets.QMainWindow):
    ...
    def addItem(self):
        ...
        self.statusBar().showMessage(f"Добавлена {row+1} строка", 0)
    ...
    def removeRow(self):
        ...
        self.statusBar().showMessage(f"Удалена {index+1} строка", 0)
    ...
    def open_file(self):
        try:
            ...
            self.load_database()
            self.statusBar().showMessage(f"Открыт файл {path}", 0)
        except FileNotFoundError:
            print("file not open!")
            self.statusBar().showMessage(f"Файл НЕ был открыт!", 0)
    ...
    def concatinate_file(self):
        try:
            ...
            self.statusBar().showMessage(f"Исходная таблица объеденена с файлом {path}", 0)
        except FileNotFoundError:
            print("file not open!")
            self.statusBar().showMessage(f"Файл НЕ был открыт!", 0)
    ...
    def create_new_file(self):
        try:
            ...
            self.load_database()
            self.statusBar().showMessage(f"Создан файл {path}", 0)
        except FileNotFoundError:
            print("file not open!")
            self.statusBar().showMessage(f"Файл НЕ создан!", 0)
    ...
    def save_file(self):
        ...
        self.statusBar().showMessage("Файл сохранен", 0)
    ...
    def saveas_file(self):
        ...
        if path != '' and path.split(".")[-1] in ("json"):
            ...
            self.statusBar().showMessage(f"Файл сохранен как {path}", 0)
        else:
            print("file not save!")
            self.statusBar().showMessage(f"Файл НЕ сохранен!", 0)
    ...
    def clear_table(self):
        ...
        self.statusBar().showMessage(f"Таблица очищена", 0)
